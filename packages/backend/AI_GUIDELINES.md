...
- **Per-user rate limiting** (Dia 37, **plan-aware since Dia 49**): every `/api/ai/*` route (and the voice assistant's `explain` intent) is capped at `AI_DAILY_RATE_LIMIT_FREE`/`AI_DAILY_RATE_LIMIT_PRO` requests/user/day depending on `plan` (defaults 1 free / 10 paid, env-overridable), enforced by `middleware/rateLimiter.js` + `services/rateLimitService.js` via a Redis fixed-window counter, with `services/planService.js` doing a fresh DB read of `plan` per request (same pattern as `requireAdmin`). Fails open (allows the request) if Redis is unreachable — same tradeoff as the cache, since a hard block on infra failure would be worse than a temporarily uncapped user. This is checked before the cache/API-key logic, so a cache hit still consumes a slot; the alternative (not counting cache hits) would let a user attempt unlimited real generations by supplying slightly different inputs that always miss cache, defeating the point of the limit. Deliberately not "unlimited for paying users" — a one-time lifetime purchase against an unbounded ongoing AI cost needs a cap regardless of plan (see `MONETIZATION.md`).
...
- **Rate limit defaults are a guess.** `AI_DAILY_RATE_LIMIT_FREE=1` / `AI_DAILY_RATE_LIMIT_PRO=10` (Dia 37, made plan-aware Dia 49) were chosen for plausibility (matching `PRICING.md`), not tuned against real usage data — there isn't any at meaningful volume yet. It's also one shared bucket across `explain`/`chat`/`remedial` combined, not per-endpoint.
