...
Body: `{ "subjectId", "variant", "metadata"? }`. Logs a conversion event for later results aggregation — used for UI-driven goals like the paywall's "Ver planos" click. 201 → `{ "recorded": true }`. 400 if `subjectId`/`variant` is missing. 404 if the experiment/variant isn't configured. The `pricing_price` experiment's real conversion (an actual purchase) isn't logged this way — see the payment webhook below, which attributes it server-side from the Mercado Pago payment metadata instead of trusting a client-reported event for something as consequential as a completed sale.

## Payment (`/api/payment`)

`TECHSPEAKING Vitalício` is a one-time purchase (Dia 46's pricing decision — see `PRICING.md`), not a subscription, so this is a single Mercado Pago Checkout Pro preference, not a recurring-billing setup. Switched from Stripe to Mercado Pago on 2026-08-11 (see `CLAUDE.md`) after a stuck Stripe identity-verification review. See `PAYMENT_TROUBLESHOOTING.md` for how to test this against a real Mercado Pago account once one exists, and common failure modes.

### `POST /api/payment/create-checkout-session` — requires auth

Body: `{ "subjectId"? }` (optional — the `pricing_price` experiment's subject id, normally the frontend's persisted anonymous visitor id so the price charged matches whatever was shown pre-login; falls back to the caller's user id if omitted). Creates a Mercado Pago Checkout Pro preference for the assigned variant's price (`metadata.user_id` set to the caller's id so the webhook below knows who to grant access to; `metadata.experiment_name`/`experiment_subject_id`/`experiment_variant` set so the webhook can attribute the eventual conversion — snake_case since Mercado Pago normalizes metadata keys to snake_case regardless of what's sent). 200 → `{ "url" }` — the frontend redirects the browser here, to the preference's `init_point`. 400 if the caller already has `plan = 'pro'`. 503 if `MERCADOPAGO_ACCESS_TOKEN` isn't configured (true in this dev environment — no real Mercado Pago account exists yet).

### `POST /api/payment/webhook`

No auth (Mercado Pago calls this directly) — authenticated instead by verifying the `x-signature`/`x-request-id` headers against `MERCADOPAGO_WEBHOOK_SECRET` (HMAC-SHA256 over `id:<data.id>;request-id:<x-request-id>;ts:<ts>;`, per Mercado Pago's own spec). Unlike Stripe's raw-byte HMAC, this only needs headers and the query string, so it's a normal JSON-parsed route (registered after the global `express.json()` middleware in `app.js`). On a `type=payment` notification whose fetched payment (`GET /v1/payments/:id`) has `status = 'approved'`, sets `users.plan = 'pro'` for `metadata.user_id`, and — if the payment carries `experiment_name`/`experiment_subject_id`/`experiment_variant` metadata — logs an experiment conversion event, attributing the real purchase to whichever pricing variant led to it. Every other notification type, and any non-approved payment status, is acknowledged (200) without action. 200 → `{ "received": true }`. 400 on an invalid signature. 503 if `MERCADOPAGO_WEBHOOK_SECRET` isn't configured.
